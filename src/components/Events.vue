<template>
	<div id="events" class="events-container">
		...
		<div class="categories">
			<ul>
				<li v-for="(category, idx) in allCategories" :key="idx"
					:class="{ 'in-active': !activeCategories.includes(category.courseTitle) }"
		...
		<div v-if="!isLoading && events.length > 0" class="events">
			<EventTile v-for="(event, idx) in filteredEvents" :key="idx" :event="event"
		...
		<div class="container center">
			<p class="center">
				Wil je automatisch alle evenementen in je agenda krijgen? Zorg ervoor dat alle richtingen waar je
				activiteiten van wilt ontvangen aan staan en klik op onderstaande knop om de link te kopiëren en
				importeer deze in je favoriete agenda applicatie!
			</p>
			<Button size="l" :center="true" url="" @click.native="copyFeedLinkToClipboard">
